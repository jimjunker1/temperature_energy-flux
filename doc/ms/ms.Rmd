...
n_dom_min = spp_dominance %>% dplyr::slice(which.min(n_dom))
n_dom_max = spp_dominance %>% dplyr::slice(which.max(n_dom))
...
## Trait distributions among sites and taxa
...
pb_summ = skew_analysis[["PB_skew_summ"]]

m_summ = skew_analysis[["M_skew_summ"]]
```

The fluxes of OM were distributed differently across body sizes and turnover rates (**P:B**) among and within streams. Across streams, mean body size of taxa ranged from `r m_summ %>% slice_min(M_mg_ind_mean) %>% select(M_mg_ind_mean)%>% round(2)` to `r m_summ %>% slice_max(M_mg_ind_mean) %>% select(M_mg_ind_mean)%>% round(2)` and mean annual P:B ranged from `r pb_summ %>% slice_min(pb_y_mean) %>% select(pb_y_mean)%>% round(2)` to `r pb_summ %>% slice_max(pb_y_mean) %>% select(pb_y_mean)%>% round(2)` (Figure 2). Fluxes were skewed differentially towards larger body sizes (positive skew) among streams, with skew estimates with body size (**M**) ranging from `r m_summ %>% slice_min(abs(M_mg_ind_skew_mean)) %>% select(M_mg_ind_skew_mean)%>% round(2)` to `r m_summ %>% slice_max(abs(M_mg_ind_skew_mean)) %>% select(M_mg_ind_skew_mean)%>% round(2)` (Figure 3). Similarly, skew in fluxes towards high turnover taxa varied among streams ranging from `r pb_summ %>% slice_min(abs(pb_y_skew_mean)) %>% select(pb_y_skew_mean)%>% round(2)` to `r pb_summ %>% slice_max(abs(pb_y_skew_mean)) %>% select(pb_y_skew_mean) %>% round(2)` (Figure 3).

When compared to random

-- mean skew with M ranged from 

-- The probability of observing these values of skew with pb `r pb_summ %>% select(matches("pb.*prob_mean")) %>% min%>% round(2)` to `r pb_summ %>% select(matches("pb.*prob_mean")) %>% max%>% round(2)`

-- The probability of observing these values of skew with M `r m_summ %>% select(matches("M.*prob_mean")) %>% min%>% round(2)` to `r m_summ %>% select(matches("M.*prob_mean")) %>% max%>% round(2)`
