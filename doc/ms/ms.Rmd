...
Variation in the relative distribution of OM fluxes among consumers corresponded to different patterns of dominance across streams (Figure 1a). For example, in an absolute sense, ~85% of total flux was contributed by `r n_dom_min$n_dom` to `r n_dom_max$n_dom` species (`r round(n_dom_min$dom_flux,2)`% to `r round(n_dom_max$dom_flux,2)`% of flux, respectively). Relatively, this flux was attributed by `r round(perc_dom_min$dom_spp*100,0)`% to `r round(perc_dom_max$dom_spp*100,0)`% of the species assemblages within streams. Differences in dominance were unrelated to temperature, however, species dominance in relation to population traits exhibited important differences along the temperature gradient.   
...
[@diaz2021] SAD are more uneven than their statistical background
