...
plot_skew_temperature <- function(temperature_stats, n_id = 2e2) {
  ...
  pb_temp_pred_plot = temperature_stats[["pb_skew_temp_pred"]] %>%
    ...
    geom_errorbar(data = pb_skew_summ, aes(x = tempC, ymin = pb_y_skew_quant2.5, ymax = pb_y_skew_quant97.5, color = site), size = 1, width = 0, inherit.aes = FALSE)+
    geom_errorbar(data = pb_skew_summ, aes(x = tempC, ymin = pb_y_skew_quant25, ymax = pb_y_skew_quant75, color = site), size = 1.5, width = 0, inherit.aes = FALSE)+ 
    geom_point(data = pb_skew_summ, aes(x = tempC, y = pb_y_skew_mean, fill = site), size = 2, color = 'black', inherit.aes = FALSE) +
