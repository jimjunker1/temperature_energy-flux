...
plot_trait_histogram <- function(spp_rankings = spp_rankings_summary) {
  ...
  pb_df = spp_rankings[['PB_spp_rank']] %>%  dplyr::filter(pb_y_mean >0) %>% dplyr::mutate(site = factor(site, levels = rev(stream_order)))
  ...
  pb_boxplot = ggplot(pb_df,aes(x = site, y = log10(pb_y_mean))) + geom_violin(aes(fill = site), alpha = 0.5) +
  ...
  M_boxplot = ggplot(M_df,aes(x = site, y = log10(M_mg_ind_mean))) + geom_violin(aes(fill = site), alpha = 0.5) +
    geom_jitter(aes( fill = site),color = 'black', size = 2, shape = 21, alpha = 0.5) +
    scale_fill_manual(values = rev(ocecolors[['temperature']][oce_temp_pos]), labels = rev(stream_temp_labels)) +
    theme(axis.title.x = element_blank(), legend.title = element_blank(), legend.position = "none")
  
  bio_boxplot = ggplot(bio_df,aes(x = site, y = log10(bio_mg_m_mean))) + geom_violin(aes(fill = site), alpha = 0.5) +
    geom_jitter(aes( fill = site),color = 'black', size = 2, shape = 21, alpha = 0.5) +
    scale_fill_manual(values = rev(ocecolors[['temperature']][oce_temp_pos]), labels = rev(stream_temp_labels)) +
    theme(axis.title.x = element_blank(), legend.title = element_blank(),legend.position = "none")
