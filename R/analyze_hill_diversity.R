...
analyze_hill_diversity <- function(ann_spp_flux_boots = flux_summaries[["annual_spp_flux_boots"]], ann_spp_flux_summary = flux_summaries[["annual_spp_flux_summary"]]) {
  ...
  ann_spp_flux = ann_spp_flux_summary %>%
    select(site, taxon, flux_mg_m_y_mean) %>%
  ...
  x = evenness_profile_function(ann_spp_flux, q.seq = seq(0, 3, 0.05))
  
  tax_q_profile(x[, , 5],seq(0, 3, 0.05), "E5")+ylim(c(0, 1))
# 
